...
The system will consist of one embedded application. The embedded application will be used to control the hardware input and output of the UAV\index{UAV}.\\\\
The embedded application will need to communicate with a GPS\index{GPS}, a gyrocompass\index{Gyrocompass}, 3 rotary encoders with a rotary encoder for each turbine.
The GPS\index{GPS} will provide a latitude, longitude and altitude to enable positioning of the aircraft.
...
The control software expects digital signals within accepted values, and relies on the mechanical design of the UAV\index{UAV} to be correctly implemented so that motors deploy mechanisms properly. These properties of the UAV\index{UAV} are expected to be implemented properly by their respective designers with respect to this software.
